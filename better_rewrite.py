...
def extract_inner_content(block, start_str):
    full_block = extract_block(block, start_str)
    if not full_block: return ""
    # strip `{condition && (` and `)}`
    # the condition ends with `&& (`
    content = full_block[len(start_str):-2]
    return content
...
"""
import re
text = re.sub(r"(const \[activeAccountingInputsModal.*?useState.*?null\);)", r"\1" + states_injection, text)

# 2. Update deep view calculations
...
text = re.sub(deepview_calc_regex, deepview_calc_replacement, text)
...
text = re.sub(handleCloseDeepView_regex, handleCloseDeepView_replacement, text)
...
text = re.sub(deps_regex, deps_replacement, text)

# 3. Rewrite Advanced Management
advanced_full = extract_block(text, "{activeTab === 'advanced-management' && (")
if advanced_full:
    db_content = extract_inner_content(advanced_full, "{advancedTab === 'database' && (")
    general_manager_content = extract_inner_content(advanced_full, "{advancedTab === 'general_manager' && (")
    system_reset_content = extract_inner_content(advanced_full, "{advancedTab === 'system_reset' && (")
    
    new_advanced = """{activeTab === 'advanced-management' && (
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-6 bg-black/80 backdrop-blur-sm">
                          ...
                          >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar relative">
                              {activeAdvancedModal === 'database' && (""" + db_content + """
                              )}
                              {activeAdvancedModal === 'general_manager' && (""" + general_manager_content + """
                              )}
                              {activeAdvancedModal === 'system_reset' && (""" + system_reset_content + """
                              )}
                            </div>
                          </motion.div>
                        </div>
    ...
    text = text.replace(advanced_full, new_advanced)

# 4. Rewrite Management
management_full = extract_block(text, "{activeTab === 'management' && (")
if management_full:
    db_read_content = extract_inner_content(management_full, "{managementTab === 'database_read' && (")
    backup_content = extract_inner_content(management_full, "{managementTab === 'backup' && (")
    devices_content = extract_inner_content(management_full, "{managementTab === 'devices' && (")
    
    new_management = """{activeTab === 'management' && (
                  <div className="space-y-6 pb-8 text-right font-cairo" dir="rtl">
                    <div className="grid grid-cols-1 gap-4 max-w-2xl mx-auto pt-4">
                      ...
                      {activeManagementModal !== null && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-6 bg-black/80 backdrop-blur-sm">
                          ...
                          >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar relative">
                              {activeManagementModal === 'database_read' && (""" + db_read_content + """
                              )}
                              {activeManagementModal === 'backup' && (""" + backup_content + """
                              )}
                              {activeManagementModal === 'devices' && (""" + devices_content + """
                              )}
                            </div>
                          </motion.div>
                        </div>
    ...
    text = text.replace(management_full, new_management)
