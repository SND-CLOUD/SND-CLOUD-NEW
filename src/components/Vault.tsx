...
export default function Vault({ user, shopConfig, onBack }: { user: User; shopConfig: ShopConfig | null; onBack: () => void }) {
  ...
  const [paymentMode, setPaymentMode] = useState<'unified' | 'different'>('unified');
  const [liabilityCurrency, setLiabilityCurrency] = useState<string>('USD');
...
شكراً لتعاملكم معنا.`;
  ...
  const handlePostVoucher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalPaymentType = paymentMode === 'unified' ? 1 : 2;
      ...
      await setDoc(doc(db, 'vault_transactions', txId), {
        ...
        amount: isReceipt ? parsedAmount : -parsedAmount,
        customerName: customerSearch.trim() || 'جهة عامة أخرى',
        invoiceNumber: '',
        ...
        liabilityCurrency: finalLiabilityCurrency,
        liabilityAmount: finalLiabilityAmount,
  ...
  const getStatementEntries = (customerId: string, currency: string) => {
    ...
    const customerTransactions = transactions.filter(tx => 
      ...
      (tx.liabilityCurrency || tx.currency || 'USD') === currency &&
      !tx.isReversed &&
      !tx.isReversal &&
      tx.status !== 'reversed' &&
      tx.status !== 'reversal'
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              {showPrintPreview ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex bg-[#1a1a1a] p-1 rounded-xl border border-white/5 mx-auto max-w-sm font-cairo">
                    <button
                      type="button"
                      onClick={() => setPaymentMode('unified')}
                      className={`flex-1 py-1.5 text-[11px] font-bold rounded-lg transition-all ${paymentMode === 'unified' ? 'bg-amber-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                      عملة موحدة
                    </button>
                    <button
                      type="button"
                      onClick={() => setPaymentMode('different')}
                      className={`flex-1 py-1.5 text-[11px] font-bold rounded-lg transition-all ${paymentMode === 'different' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                      عملة مختلفة
                    </button>
                  </div>
                  ...
                  <div className="max-w-xl mx-auto bg-[#141414] border border-white/5 rounded-2xl p-3.5 space-y-1.5 font-cairo">
                     ...
                     {/* Liability Fields for Different Currency */}
                     {paymentMode === 'different' && (
                       <>
                         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-4 py-1 border-b border-white/5">
                           <label className="text-[11px] font-bold text-gray-400 min-w-[120px]">
                             عملة الذمة (الحساب):
                           ...
                           <div className="flex-1 max-w-xs relative text-right">
                              <input 
                                ...
                                onChange={(e) => {
                                  ...
                                  setLiabilityAmount(val);
                                }}
                                className="w-full bg-black/40 border border-white/5 rounded-xl px-3 py-1 text-xs text-blue-400 font-bold font-mono outline-none focus:border-blue-500/50 placeholder:text-gray-800 text-left"
                                required
                              />
                              <span className="absolute right-3 top-1 text-[9px] text-gray-500 font-bold">{liabilityCurrency}</span>
                           </div>
                         </div>
                       </>
                     )}
